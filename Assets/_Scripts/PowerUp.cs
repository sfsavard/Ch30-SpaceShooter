...
public class PowerUp : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        vel *= Random.Range(driftMinMax.x, driftMinMax.y);                   // a Sets the velocity length to something between the x and y values of driftMinMax
        rigid.velocity = vel;
    ...
    void Update()
    {
        cube.transform.rotation = Quaternion.Euler(rotPerSecond * Time.time);// b Manually rotate the Cube child every Update(). Multiplying rotPerSecond by Time.time causes the rotation to be time-based.
        ...
        if (u > 0)
        {
            Color c = cubeRend.material.color; c.a = 1f - u;
